        ...
        [
            ...
            'value' => function ($data) {
                ...
                if($posMFP === false) {
                    ...
                    $numAct = ActData::find()->where(['AND', ['type' => $type], ['company' => $company], ['period' => $period]])->andWhere('`name` LIKE "%МФП%"')->andWhere('`name` LIKE "%' . $data->number . '%"')->andWhere('`name` LIKE "%' . $data->act_id . '%"')->select('number')->column();
                }
                ...
                'search' => function ($url, $model, $key) {
                    ...
                    if($GLOBALS['company']) {
                        return Html::a('<span class="glyphicon glyphicon-search"></span>',
                            $prefixHttp . Url::to('@frontWeb') . '/act/list?' . urlencode('ActSearch[period]') . '=' . $GLOBALS['period'] . '&' . urlencode('ActSearch[client_id]') . '=' . $model->client_id . '&' . urlencode('ActSearch[car_number]') . '=' . $model->number . '&type=' . $GLOBALS['type'] . '&company=1', ['target' => '_blank']);
                    } else {
                        return Html::a('<span class="glyphicon glyphicon-search"></span>',
                            $prefixHttp . Url::to('@frontWeb') . '/act/list?' . urlencode('ActSearch[period]') . '=' . $GLOBALS['period'] . '&' . urlencode('ActSearch[partner_id]') . '=' . $model->client_id . '&' . urlencode('ActSearch[car_number]') . '=' . $model->number . '&type=' . $GLOBALS['type'], ['target' => '_blank']);
                    }
