...
$filters = ($company == 1 ? 'Выбор компании: ' : 'Выбор партнера: ') . Html::activeDropDownList($searchModel, ($company == 1 ? 'client_id' : 'partner_id'), Company::find()->active()
        ->andWhere(['company.type' => ($company == 1 ? $company : $type)])->innerJoin('act', ($company == 1 ? 'act.client_id = company.id' : 'act.partner_id = company.id'))
        ->andWhere(['service_type' => $type])
        ->andWhere(['between', 'act.served_at', strtotime($searchModel->dateFrom), strtotime($searchModel->dateTo)])
        ->select(['company.name', 'company.id'])->indexBy('id')
        ->orderBy(($company == 1 ? 'act.client_id ASC' : 'act.partner_id ASC'))
        ->column(), ['prompt' => 'все', 'class' => 'form-control ext-filter', 'style' => 'width: 200px; margin-right: 10px']);
...
if($company != 1) {
    ...
    $columns = [
        [
            'attribute' => 'client_id',
            'content' => function ($data) {
                return $data->client->name . ' - ' . $data->client->address;
            },
            'group' => true,
            'groupedRow' => true,
            'groupOddCssClass' => 'kv-group-header',
...
echo GridView::widget([
    ...
    'panel' => [
        'type' => 'primary',
        'heading' => $company != 1 ? ('Статистика компаний по партнерам: ' . Company::$listType[$type]['ru']) : ('Статистика партнеров по компаниям: ' . Company::$listType[$type]['ru']),
        'before' => false,
        'footer' => false,
        'after' => false,
    ],
